...
import { Client, ClientStatusEnum, LaunchOpts } from '../../types/client'
...
const launchHandler = async (event: IpcMainEvent, opts: LaunchOpts) => {
	try {
		const { client, version } = opts
		...
		const gamePath = join(installationPath, client)
		const javaPath = join(
			installationPath,
			client,
			'runtime',
		...
		await launch({
			...
			spawn: (command, args) => {
				event.reply(`core/launch/client/${client}`, {
